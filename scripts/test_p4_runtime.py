...
with tempfile.TemporaryDirectory(prefix='k1-p4-runtime-') as temp:
    ...
    run(['c++','-std=c++17','-O2','-ffp-contract=off','-fno-fast-math','-DK1_NPU_LOAD=1','-DK1_P4_LOAD=1',
         ...
         str(ROOT/'platform/ra8p1/semantic_sidecar.cpp'),str(args.kernels/'kernels.c'),'-o',str(executable)])
